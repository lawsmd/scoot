...
-- _RestorePosition and _ShowEditModePreview live in editmode.lua; _ApplyStack in
-- stack.lua. All are resolved at drain time, so their load order behind this
...
local function ensureFrame(inst)
    ...
    -- Ping receiver and its name-row proxy: ping.lua, which loads after this
    -- file and is reached at call time.
    UFZ._BuildPingReceiver(inst)
...
--------------------------------------------------------------------------------
-- Reset: the full worker list from defaults (API.Reset; every other setter
-- lives in setters.lua or auras.lua, registered into UFZ._API)
--------------------------------------------------------------------------------
