...
function SlashCmdList.SCOOT(msg, editBox)
    ...
    if cmd == "debug" then
        ...
        if sub1 == "" then
            ...
            addon:Print("  /scoot debug healthtext [unit|font|valfont|valsquish|style|pct|val|digitsize|gap|center|centeroffset|digits|digitprobe|descent|stretch|symbol|align|color|round|namesize|namefont|nameoffset|namey|chrome|reset|report|measure|probe]")
        ...
        -- /scoot debug healthtext - the Unit Frames Z health block, built as it would ship
        if sub1 == "healthtext" then
            if not addon.DebugHealthTextToggle then
                addon:Print("Health text debug not loaded.")
                return
            end
            if sub2 == "" then
            ...
            if sub2 == "unit" then
                addon.DebugHealthTextSetUnit(args[4])
                return
            end
            if sub2 == "font" then
                -- args[4] raw: font keys are case-sensitive (e.g. ANTON_WIDE_180)
                addon.DebugHealthTextSetFont(args[4])
                return
            end
            if sub2 == "style" then
                -- args[4] raw: style strings are uppercase (e.g. SHADOWOUTLINE)
                addon.DebugHealthTextSetStyle(args[4])
                return
            end
            if sub2 == "pct" then
                addon.DebugHealthTextSetPctSize(args[4])
                return
            end
            if sub2 == "val" then
                addon.DebugHealthTextSetValSize(args[4])
                return
            end
            if sub2 == "valfont" then
                -- args[4] raw: font keys are case-sensitive; 'follow' tracks the main face
                addon.DebugHealthTextSetValFont(args[4])
                return
            end
            if sub2 == "valsquish" then
                addon.DebugHealthTextSetValSquish(args[4])
                return
            end
            if sub2 == "descent" then
                addon.DebugHealthTextSetDescent(args[4])
                return
            end
            if sub2 == "gap" then
                addon.DebugHealthTextSetGap(args[4])
                return
            end
            if sub2 == "center" then
                addon.DebugHealthTextSetCenter(args[4])
                return
            end
            if sub2 == "centeroffset" then
                addon.DebugHealthTextSetCenterOffset(args[4])
                return
            end
            if sub2 == "digits" then
                addon.DebugHealthTextSetDigits(args[4])
                return
            end
            if sub2 == "digitsize" then
                addon.DebugHealthTextSetDigitSize(args[4], args[5])
                return
            end
            if sub2 == "digitprobe" then
                addon.DebugHealthTextDigitProbe()
                return
            end
            if sub2 == "stretch" then
                addon.DebugHealthTextSetStretch(args[4])
                return
            end
            if sub2 == "symbol" then
                addon.DebugHealthTextSetSymbol(args[4], args[5])
                return
            end
            if sub2 == "align" then
                addon.DebugHealthTextSetAlign(args[4])
                return
            end
            if sub2 == "color" then
                addon.DebugHealthTextSetColor(args[4])
                return
            end
            if sub2 == "round" then
                addon.DebugHealthTextSetRound(args[4])
                return
            end
            if sub2 == "namesize" then
                addon.DebugHealthTextSetNameSize(args[4])
                return
            end
            if sub2 == "namefont" then
                -- args[4] raw: font keys are case-sensitive; 'follow' tracks the main face
                addon.DebugHealthTextSetNameFont(args[4])
                return
            end
            if sub2 == "nameoffset" then
                addon.DebugHealthTextSetNameOffset(args[4])
                return
            end
            if sub2 == "namey" then
                addon.DebugHealthTextSetNameY(args[4])
                return
            end
            if sub2 == "chrome" then
                addon.DebugHealthTextToggleChrome()
                return
            end
            if sub2 == "reset" then
                addon.DebugHealthTextReset()
                return
            end
            if sub2 == "report" then
                addon.DebugHealthTextReport()
                return
            end
            if sub2 == "measure" then
                addon.DebugHealthTextMeasure()
                return
            end
            if sub2 == "probe" then
                addon.DebugHealthTextProbe()
                return
            end
            addon:Print("Usage: /scoot debug healthtext            (show/hide)")
            addon:Print("       /scoot debug healthtext unit <player|target|focus>")
            addon:Print("       /scoot debug healthtext font <FACE>")
            addon:Print("       /scoot debug healthtext style <STYLE>   (e.g. SHADOWOUTLINE, NONE)")
            addon:Print("       /scoot debug healthtext pct <size> | val <size> | gap <px>   (val takes 0.5 steps; gap 0.1, negative ok)")
            addon:Print("       /scoot debug healthtext valfont <FACE|follow> | valsquish <90|80|70|off>   (value row face; squish follows the main face's family)")
            addon:Print("       /scoot debug healthtext center <on|off> | centeroffset <px>   (shared centerline)")
            addon:Print("       /scoot debug healthtext digits <on|off> | digitsize <1|2|3> <size> | digitprobe")
            addon:Print("       /scoot debug healthtext descent <ratio>   (pins the percent baseline; keeps the row gap constant)")
            addon:Print("       /scoot debug healthtext namesize <size> | namefont <FACE|follow> | nameoffset <px> | namey <px>")
            addon:Print("       /scoot debug healthtext stretch <factor>   (width-only, 1 = off)")
            addon:Print("       /scoot debug healthtext symbol <on|off> [size]   (small '%' glyph)")
            addon:Print("       /scoot debug healthtext align <right|left>")
            addon:Print("       /scoot debug healthtext color <curve|dark|white>")
            addon:Print("       /scoot debug healthtext round <floor|round>")
            addon:Print("       /scoot debug healthtext chrome            (backdrop on/off, to drag the box)")
            addon:Print("       /scoot debug healthtext reset | report | measure | probe")
            return
        end
