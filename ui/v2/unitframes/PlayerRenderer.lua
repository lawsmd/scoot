...
function UF.RenderPlayer(panel, scrollContent)
    ...
    Sections.BuildParentControls(B, {
        ...
        -- Reverting to Blizzard art also drops the custom power bar height.
        useCustomBorders = { onDisable = function(t, wasEnabled)
            if wasEnabled then t.powerBarHeightPct = 100 end
        end },
    })
    ...
    Sections.BuildNameLevelTextSection(B, {
        builder = builder, componentId = COMPONENT_ID,
        nameText = { hideToggleInBlock = true },
    })
    ...
    Sections.BuildPortraitSection(B, {
        builder = builder, componentId = COMPONENT_ID,
        fullCircleMask = true, personalText = true,
    })
