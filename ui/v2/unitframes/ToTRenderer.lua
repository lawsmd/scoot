-- ToTRenderer.lua - Target of Target and Focus Target TUI renderers
-- Neither frame is in Edit Mode, so both have addon-controlled positioning and
-- scale. One factory builds both pages: the two differ only in the tokens in
-- UNITS (the db key, the three appliers, and the label), so the body is
-- written once.
...
local UNITS = {
    {
        componentId = "ufToT", unitKey = "TargetOfTarget", label = "Target of Target",
        applyPosition = "ApplyToTPosition", applyScale = "ApplyToTScale", applyNameText = "ApplyToTNameText",
    },
    {
        componentId = "ufFocusTarget", unitKey = "FocusTarget", label = "Target of Focus",
        applyPosition = "ApplyFocusTargetPosition", applyScale = "ApplyFocusTargetScale", applyNameText = "ApplyFoTNameText",
    },
}

-- The appliers are resolved by name at call time, as the guards always were.
local function callApplier(name)
    local fn = addon[name]
    if fn then fn() end
end
...
local function CreateRenderer(unit)
    ...
    local function applyPosition()
        callApplier(unit.applyPosition)
    end

    local function applyScale()
        callApplier(unit.applyScale)
    end
    ...
    local function applyNameText()
        callApplier(unit.applyNameText)
        B.applyStyles()
    end
    ...
    local function render(panel, scrollContent)
        ...
        builder:AddSlider({
            label = "Scale",
            description = "Overall scale of the " .. unit.label .. " frame.",
